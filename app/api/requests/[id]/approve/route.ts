...
export async function POST(
  ...
  try {
    ...
    const updatedRequest = await RequestRepository.updateRequestStatus(requestId, "APPROVED");
    ...
    const notif = await NotificationRepository.createNotification({
      ...
      message: `Admin approved request for team "${(requestItemTeam as any)?.name || "N/A"}".`,
